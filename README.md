...
```bash
# Историческая проверка ML: загрузить CSV только при отсутствии artifacts/demo.db:
...
Старое тестовое демо с отдельными `backend/prediction_worker.py`, `data/ingestion.py`, `data/storage.py` и `ml/pipeline.py` удалено. Исторический режим выше использует актуальные ML-модели и общую базу, поэтому сохраняется как самостоятельная проверка без эмулятора.
