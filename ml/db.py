...
def predictions_table(cfg: DBConfig):
    ...
    return Table(
        ...
        UniqueConstraint("tr_id", "t_forecast", "target_stop_id", name="uq_predictions_vehicle_time_stop"),
    )
...
def write_predictions(engine, cfg: DBConfig, preds) -> int:
    ...
    if not rows:
        return 0
    # Worker и ручной /predict/from-db могут запустить один и тот же цикл.
    if engine.dialect.name == "sqlite":
        from sqlalchemy.dialects.sqlite import insert
    elif engine.dialect.name == "postgresql":
        from sqlalchemy.dialects.postgresql import insert
    else:
        raise ValueError("Для защиты от дублей поддерживаются SQLite и PostgreSQL")
    statement = insert(tbl).values(rows).on_conflict_do_nothing(
        index_elements=["tr_id", "t_forecast", "target_stop_id"]
    )
